		...
		public void actionPerformed(ActionEvent event){
			...
			case"Editar":
				try {
					...
					int linha1 =tabela.seletctID();
				   		limparTabela();
				   		PizzasDTO p =ControlPizzas.pesquisarPizza(linha1);
				   		cod.setText(""+p.getCod());
				   		boxnome.setText(p.getNome().toUpperCase());
				   		boxtamanho.setSelectedItem(p.getTamnaho());
				   		boxquant.setText(""+p.getQuantft());
				   		precof.setText(""+p.getValorft());
				   		precot.setText(""+p.getValorcomp());
				   		preparo.setText(p.getPrepara().toUpperCase());
				   		precouni.setText(""+p.getCusto());
		    ...
		    if(linha1<0) {
		    	JOptionPane.showMessageDialog(null, "Selecione a linha pra exclusão", "Aviso", JOptionPane.ERROR_MESSAGE);	}	
		    		...
		    		ControlPizzas.ExcluirPizza(linha1);
